...
default_formats = {
    # 终端输出格式
    'color_format': '%(log_color)s%(asctime)s-%(name)s-%(filename)s-%(levelname)s-[日志信息]: %(message)s',
    # 日志输出格式
    'log_format': '%(asctime)s-%(name)s-%(filename)s-%(levelname)s-[日志信息]: %(message)s'
}
...
class HandleLog:
    ...
    def __console(self, level: str, message: str, exc_info=False, extra={}, stack_info=False):
        """构造日志收集器"""
        ...
        if level == 'info':
            ...
            self.__logger.error(message, exc_info=exc_info, extra=extra, stack_info=stack_info)
        elif level == 'critical':
            self.__logger.critical(message, exc_info=exc_info, extra=extra, stack_info=stack_info)
